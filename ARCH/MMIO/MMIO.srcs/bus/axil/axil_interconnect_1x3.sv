...
module axil_interconnect_1x3(
    ...
    // AXI-Lite master port to ASCON
    output logic [31:0] m_ascon_awaddr,
    output logic        m_ascon_awvalid,
    input  logic        m_ascon_awready,

    output logic [31:0] m_ascon_wdata,
    output logic [3:0]  m_ascon_wstrb,
    output logic        m_ascon_wvalid,
    input  logic        m_ascon_wready,

    input  logic [1:0]  m_ascon_bresp,
    input  logic        m_ascon_bvalid,
    output logic        m_ascon_bready,

    output logic [31:0] m_ascon_araddr,
    output logic        m_ascon_arvalid,
    input  logic        m_ascon_arready,

    input  logic [31:0] m_ascon_rdata,
    input  logic [1:0]  m_ascon_rresp,
    input  logic        m_ascon_rvalid,
    output logic        m_ascon_rready
);
    localparam logic [2:0] SEL_NONE  = 3'd0;
    localparam logic [2:0] SEL_RAM   = 3'd1;
    localparam logic [2:0] SEL_UART  = 3'd2;
    localparam logic [2:0] SEL_ASCON = 3'd3;
    localparam logic [2:0] SEL_ERR   = 3'd4;
    ...
    function automatic logic [2:0] decode_sel(input logic [31:0] addr);
        if (((addr >= `SOC_RAM_BASE) && (addr < (`SOC_RAM_BASE + `SOC_RAM_SIZE))) || (addr < `SOC_RAM_SIZE)) begin
            ...
            decode_sel = SEL_UART;
        end else if ((addr >= `SOC_ASCON_BASE) && (addr < (`SOC_ASCON_BASE + `SOC_ASCON_SIZE))) begin
            decode_sel = SEL_ASCON;
        end else begin
            decode_sel = SEL_ERR;
        end
    endfunction
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (wr_aw_captured && wr_w_captured && !wr_busy) begin
                ...
                wr_aw_sent    <= 1'b0;
                wr_w_sent     <= 1'b0;
                wr_err_bvalid <= (decode_sel(wr_awaddr_reg) == SEL_ERR);
            end

            if (wr_busy && (wr_sel == SEL_RAM || wr_sel == SEL_UART || wr_sel == SEL_ASCON)) begin
                if (!wr_aw_sent) begin
                    if (wr_sel == SEL_RAM && m_ram_awready) wr_aw_sent <= 1'b1;
                    if (wr_sel == SEL_UART && m_uart_awready) wr_aw_sent <= 1'b1;
                    if (wr_sel == SEL_ASCON && m_ascon_awready) wr_aw_sent <= 1'b1;
                end

                if (!wr_w_sent) begin
                    if (wr_sel == SEL_RAM && m_ram_wready) wr_w_sent <= 1'b1;
                    if (wr_sel == SEL_UART && m_uart_wready) wr_w_sent <= 1'b1;
                    if (wr_sel == SEL_ASCON && m_ascon_wready) wr_w_sent <= 1'b1;
                end
            end

            if (wr_busy && wr_sel == SEL_ERR) begin
                ...
                wr_aw_sent     <= 1'b0;
                wr_w_sent      <= 1'b0;
            end else if (wr_busy && wr_sel == SEL_ASCON && wr_aw_sent && wr_w_sent && m_ascon_bvalid && s_axi_bready) begin
                wr_busy        <= 1'b0;
                wr_aw_captured <= 1'b0;
                wr_w_captured  <= 1'b0;
                wr_sel         <= SEL_NONE;
            ...
            if (rd_ar_captured && !rd_busy) begin
                ...
                rd_busy      <= 1'b1;
                rd_ar_sent   <= 1'b0;
                rd_err_rvalid<= (decode_sel(rd_araddr_reg) == SEL_ERR);
                rd_err_rdata <= 32'hDEAD_BEEF;
            end

            if (rd_busy && (rd_sel == SEL_RAM || rd_sel == SEL_UART || rd_sel == SEL_ASCON) && !rd_ar_sent) begin
                if (rd_sel == SEL_RAM && m_ram_arready) rd_ar_sent <= 1'b1;
                if (rd_sel == SEL_UART && m_uart_arready) rd_ar_sent <= 1'b1;
                if (rd_sel == SEL_ASCON && m_ascon_arready) rd_ar_sent <= 1'b1;
            end

            if (rd_busy && rd_sel == SEL_ERR) begin
                ...
                rd_sel         <= SEL_NONE;
                rd_ar_sent     <= 1'b0;
            end else if (rd_busy && rd_sel == SEL_ASCON && rd_ar_sent && m_ascon_rvalid && s_axi_rready) begin
                rd_busy        <= 1'b0;
                rd_ar_captured <= 1'b0;
                rd_sel         <= SEL_NONE;
                rd_ar_sent     <= 1'b0;
    ...
    always_comb begin
        ...
        m_ascon_awaddr  = wr_awaddr_reg;
        m_ascon_awvalid = wr_busy && (wr_sel == SEL_ASCON) && !wr_aw_sent;
        m_ascon_wdata   = wr_wdata_reg;
        m_ascon_wstrb   = wr_wstrb_reg;
        m_ascon_wvalid  = wr_busy && (wr_sel == SEL_ASCON) && !wr_w_sent;
        m_ascon_bready  = wr_busy && (wr_sel == SEL_ASCON) && wr_aw_sent && wr_w_sent && s_axi_bready;
        ...
        m_ascon_araddr  = rd_araddr_reg;
        m_ascon_arvalid = rd_busy && (rd_sel == SEL_ASCON) && !rd_ar_sent;
        m_ascon_rready  = rd_busy && (rd_sel == SEL_ASCON) && rd_ar_sent && s_axi_rready;
        ...
        end else if (wr_busy && wr_sel == SEL_ASCON) begin
            s_axi_bresp  = m_ascon_bresp;
            s_axi_bvalid = wr_aw_sent && wr_w_sent && m_ascon_bvalid;
        end else if (wr_busy && wr_sel == SEL_ERR) begin
            s_axi_bresp  = 2'b10;
        ...
        end else if (rd_busy && rd_sel == SEL_ASCON) begin
            s_axi_rdata  = m_ascon_rdata;
            s_axi_rresp  = m_ascon_rresp;
            s_axi_rvalid = rd_ar_sent && m_ascon_rvalid;
        end else if (rd_busy && rd_sel == SEL_ERR) begin
