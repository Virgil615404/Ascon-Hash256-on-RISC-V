...
class NegativeTestGenerator:
    ...
    def test_unaligned_lw(self):
        ...
        # Setup: Put base address in x1 (address 0)
    ...
    def test_unaligned_sw(self):
        ...
        self.add_instr(encode_i_type(1, 0, 3, 0, 0x13))  # addi x3, x0, 1
    ...
    def test_memory_stress(self):
        ...
        for i in range(16):
            ...
            self.add_instr(encode_u_type((pattern >> 12) & 0xFFFFF, 10, 0x37))  # lui x10, imm
            self.add_instr(encode_i_type(pattern & 0xFFF, 10, 0, 10, 0x13))      # addi x10, x10, imm
            
            # Store to memory[base + i*4]
            addr_reg = 2
            self.add_instr(encode_i_type(base_offset + i*4, 0, 0, addr_reg, 0x13))  # addi x(addr), x0, offset
            self.add_instr(encode_s_type(0, 10, addr_reg, 2, 0x23))  # sw x10, 0(x(addr))
        
        # Read back and verify
        for i in range(16):
            addr_reg = 2
            self.add_instr(encode_i_type(base_offset + i*4, 0, 0, addr_reg, 0x13))
            rd = 20 + i
            self.add_instr(encode_i_type(0, addr_reg, 2, rd, 0x03))  # lw x(rd), 0(x(addr))
    ...
    def test_byte_halfword_alignment(self):
        ...
        # Assume we can write to address 0x1000
        self.add_instr(encode_u_type(0x1000 >> 12, 1, 0x37))  # lui x1, 0x1000>>12
        self.add_instr(encode_i_type(0x1000 & 0xFFF, 0, 0, 1, 0x13))  # addi if needed
        
        # Write pattern 0xAABBCCDD
        self.add_instr(encode_u_type(0xAABBCCDD >> 12, 2, 0x37))
        self.add_instr(encode_i_type(0xAABBCCDD & 0xFFF, 2, 0, 2, 0x13))
