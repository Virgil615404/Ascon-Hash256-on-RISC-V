module cpu_core_soc(
    ...
    assign imem_addr  = PC;
    assign imem_flush = branch_taken_ex;
    ...
    ID_EX u_id_ex (
        .clk            (clk),
        .rst_n          (rst_n),
        .flush          (branch_taken_ex || hazard_stall || if_stall),
        .stall          (mem_stall),
    ...
    assign alu_src1 = (forwardA == 2'b10) ? (memread_mem ? mem_data_mem : alu_result_mem) :
                      (forwardA == 2'b01) ? write_data_wb :
                      read_data1_ex;

    wire [31:0] alu_src2_pre = (forwardB == 2'b10) ? (memread_mem ? mem_data_mem : alu_result_mem) :
                               (forwardB == 2'b01) ? write_data_wb :
                               read_data2_ex;
    assign alu_src2 = ALUsrc_ex ? imm_ex : alu_src2_pre;

    assign write_data_ex = (forwardB == 2'b10) ? (memread_mem ? mem_data_mem : alu_result_mem) :
                           (forwardB == 2'b01) ? write_data_wb :
                           read_data2_ex;
